...
graph TD
    subgraph "User code"
        US["MyService : DeviceMonitor"]
        UC["MyClient : DeviceMonitor"]
    end

    subgraph "Generated code (ipcgen)"
        SK["DeviceMonitor : ServiceBase (server)"]
        CL["DeviceMonitor : ClientBase (client)"]
    end
...
classDiagram
    ...
    class DeviceMonitor_Server {
        <<generated by ipcgen>>
        +kServiceId : uint32_t
        #handleGetDeviceCount(count)* int
        #handleGetDeviceStatus(deviceId, status)* int
        #notifyDeviceConnected(deviceId) int
        #notifyDeviceDisconnected(deviceId) int
        #onRequest() int
    }
    ...
    ServiceBase <|-- DeviceMonitor_Server : inherits
    DeviceMonitor_Server <|-- MyDeviceService : inherits
    ...
    class DeviceMonitor_Client {
        <<generated by ipcgen>>
        +kServiceId : uint32_t
        +GetDeviceCount(count, timeoutMs) int
        +GetDeviceStatus(deviceId, status, timeoutMs) int
        #onDeviceConnected(deviceId) void
        #onDeviceDisconnected(deviceId) void
        #onNotification() void
    }

    ClientBase <|-- DeviceMonitor_Client : inherits
...
timeline
    ...
    Phase 4 (done) : Code generation (ipcgen)
                   : IDL parser + C++ emitter
                   : server/client class generation
                   : 11 codegen C++ tests
                   : 45 Python unit tests
    Future : Struct/enum IDL types
           : Array/string support
           : Multi-service IDL files
```
