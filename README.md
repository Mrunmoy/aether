...
graph TD
    subgraph "User / Generated"
        GEN["DeviceMonitor : ServiceBase\nDeviceMonitor : ClientBase\n(generated by ipcgen)"]
    end
...
- IDL code generator (`tools/ipcgen/`) — generates typed server and client classes from `.idl` files
- Embedded-friendly: no `std::string`, no heap allocations in platform layer
- 61 C++ tests + 45 Python tests (106 total)
...
tools/ipcgen/           IDL-to-C++ code generator (Python package)
  types.py              Type map, FNV-1a hash
  lexer.py              Tokenizer
  parser.py             Recursive-descent parser, AST
  emitter.py            C++ server/client code emitter
  __main__.py           CLI entry point
  test/                 Python unit tests (45 tests, pytest)
example/                Echo examples + generated code tests
  DeviceMonitor.idl     Sample IDL file
  gen/                  Generated server/ and client/ code
  CodeGenServerTest.cpp Generated server dispatch tests (6 tests)
  CodeGenClientTest.cpp Generated client RPC tests (5 tests)
deps/ms-ringbuffer/     SPSC ring buffer (submodule)
deps/ms-runloop/        Event loop for fd watching (submodule)
test/                   Core unit tests (see test/README.md)
doc/                    Design walkthroughs
```

## Code generation

The `tools/ipcgen/` package generates typed C++ server and client classes from
a minimal IDL file. Each service produces four files in `server/` and `client/`
subdirectories.
...
See [example/DeviceMonitor.idl](example/DeviceMonitor.idl) for the IDL syntax
and [example/README.md](example/README.md) for the generated code walkthrough.
...
- [test/README.md](test/README.md) — core test organization
- [example/README.md](example/README.md) — examples and generated code tests
